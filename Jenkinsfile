pipeline {
    ...
    environment {
        ...
        DOCKERHUB_IMAGE = 'ikoyski/webtools-bin-lookup:latest'        
        DEPLOYMENT_FILENAME = 'Deploy-webtools-bin-lookup-private.yaml'
    }
    stages {
        stage('Git Stuff') {
            steps {
                script {
                	checkout scmGit(
                		...
                		extensions: [], 
                		userRemoteConfigs: [[url: 'https://github.com/ikoyski/webtools-bin-lookup.git']]
                	)
                }
            }
        }
